package io.github.jtpadilla.genai.generate;
...
public class GenerateContentWrapper {
    ...
    static public GenerateContentWrapper create(GenerateContentResponse response) throws GenerateContentException {
        ...
        final Candidate candidate = switch (candidateList.size()) {
            case 0 ->
                    throw new GenerateContentException(response.getClass().getCanonicalName() + " candidates count is 0");
            case 1 -> candidateList.getFirst();
            default ->
                    throw new GenerateContentException(response.getClass().getCanonicalName() + " too many candidates");
        };
